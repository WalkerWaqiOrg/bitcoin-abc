...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total rrcoin "
...
static UniValue validateaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given rrcoin address.\n"
            "\nArguments:\n"
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are rrcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) rrcoin address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "rrcoin-cli and the GUI. There is no server-side difference.");
    }

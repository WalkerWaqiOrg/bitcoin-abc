UNIX BUILD NOTES
====================
Some notes on how to build RRCoin in Unix.
...
This will build rrcoin-qt as well if the dependencies are met.
...
memory available when compiling RRCoin. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build RRCoin without wallet.
...
If you want to build rrcoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a rrcoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip rrcoind" to strip the debug
...
# Configure RRCoin to use our own-built instance of BDB
cd $BITCOIN_ROOT
./autogen.sh

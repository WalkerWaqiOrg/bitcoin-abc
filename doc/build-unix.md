UNIX BUILD NOTES
====================
Some notes on how to build RR CHAIN in Unix.
...
This will build rrc-qt as well if the dependencies are met.
...
memory available when compiling RR CHAIN. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build RR CHAIN without wallet.
...
If you want to build Rrc-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a rrc-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip rrcd" to strip the debug
...
# Configure RR CHAIN to use our own-built instance of BDB
cd $BITCOIN_ROOT
./autogen.sh
